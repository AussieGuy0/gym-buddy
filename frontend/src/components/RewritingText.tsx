...
export const RewritingText: React.FC<RewritingTextProps> = (props) => {
    ...
    let [replacementState, setReplacementState] = useState<ReplacementState>({replacement: getRandomReplacement(), currentIndex: -1, toAdd: true, hold: defaultHold})
    ...
    function createText(): string {
        ...
        return template.replace("$REPLACE", getReplacementText())
    }
    ...
    function determineNextState(state: ReplacementState): ReplacementState {
        ...
        if (!toAdd && currentIndex === 0) {
            return {replacement: getRandomReplacement(), currentIndex: 0, toAdd: true, hold: defaultHold}
    ...
    return (
        <span>
            {text}<span className={isStationary(replacementState) ? "blinking" : ""}>|</span>
        </span>
    )
}
